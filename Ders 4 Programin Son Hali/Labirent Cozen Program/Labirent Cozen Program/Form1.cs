namespace Labirent_Cozen_Program
{
    public partial class Form1 : Form
    {
        ...
        private void varsayilan_btn_Click(object sender, EventArgs e)
        {
            labirent.Labirent_Getir("");
        }
        ...
        private void kaydet_btn_Click(object sender, EventArgs e)
        {
            ...
            for (int y = 1; y < 29; y++)
            {
                for (int x = 1; x < 29; x++)
                {
                    if (labirent.Labirent_VeriAl(x, y) == -1)
                    {
                        labirentVeri += 1;
                    }
                    else
                    {
                        labirentVeri += labirent.Labirent_VeriAl(x, y);
                    }
                }
            }
            SaveFileDialog file = new SaveFileDialog();
            file.FileName = "labirent.txt";
            file.Filter = "txt files (*.txt)|*.txt|All files (*.*)|*.*";
            file.ShowDialog();

            using (StreamWriter dosyayaz = new StreamWriter(file.FileName))
            {
                dosyayaz.WriteLine(String.Format(labirentVeri));
            }
        }

        private void getir_btn_Click(object sender, EventArgs e)
        {
            OpenFileDialog dosya =new OpenFileDialog();
            dosya.InitialDirectory= @"C:\txt";
            dosya.Title = "sadece txt dosyalari goster";
            dosya.Filter = "Text Dosyalari  (*.txt) | *.txt";
            dosya.DefaultExt = "txt";
            if(dosya.ShowDialog()== DialogResult.OK)
            {
                string labirentVeri= File.ReadAllText(dosya.FileName);
                labirent.Labirent_Getir(labirentVeri);
            }
        }
    }
}